...
首頁另有獨立的「自製轉盤」：展開後逐個新增或刪除 2–20 個選項，等機率抽選，可重複轉動。選項僅留在本次頁面，重新整理即清空；不需建房，也不寫入用餐紀錄。
...
4. **搜尋** — 房主開始搜尋，全員條件即刻凍結。房主可開「菜系過濾」讓全員偏好聯集成為入池門檻，並用探索檔位（熟悉／平均／探索新店）調整新店與熟店的權重。候選帶著機率與逐因素解釋出現，被排除的店也列出原因；開始投票前，房主可按「修改條件」回到 lobby，清除候選並讓全員重新準備後再搜尋。房間、成員、邀請碼與既有曝光統計保留。
...
flowchart TB
  ...
  subgraph G["Go API — server/（:8787）"]
    ...
    H["handlers<br/>search / edit-conditions / start-voting / vote / draw / leave"]
...
票數）由 RLS 逐列把關，改動經 Realtime 即時推播給同房其他人；後者（搜尋、修改
候選條件、開始投票、投票、抽選、退房）需要凍結成員條件、跑過濾與加權、留下可重播的 seed，或
