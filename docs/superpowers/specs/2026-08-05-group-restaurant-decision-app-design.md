...
| `POST /api/rooms/{id}/rescore` | 任一成員 | 投票/否決（含收回）後重算全房機率寫回 `room_candidates`（idempotent），Realtime 推給全員；轉盤上的 % 永遠是當下真實機率 | 2 |
...
| `votes` | `room_id`、`user_id`、`restaurant_id`、`kind`（`up/veto`）、unique(room_id, user_id, restaurant_id, kind) | 否決限額 = **現存**否決數（每人每房同時最多 2 個）；voting 期間可 DELETE 自己的列收回，trigger 以現存列數把關 |
...
5. **正規化與抽選** — `p_i = score_i / Σscore`；轉盤 UI 顯示的就是這個真實機率（%）。P2 起每次投票/否決/收回都觸發 `rescore` 重算。抽選前 Go 權威重算一次，`crypto/rand` 加權抽樣；`draws` 存 seed 與機率快照。
...
- `voting`（P2）：投票、否決與收回，每個動作觸發 `rescore`，機率即時更新
...
- RLS：房間資料僅成員可讀；`room_members`、`votes` 只能寫（含刪除）自己的列；`rooms` 僅房主可更新（含 `exploration` 檔位，於 `lobby` 階段調整）；join 需正確邀請碼且房間在 `lobby`
...
API 層少量 `httptest`（JWT 驗證、房間 happy path、rescore idempotency）；前端僅對機率/chips 渲染邏輯寫一個 vitest，其餘走手動 demo script。
...
- **Phase 2（群組互動）**：投票、有限否決權（現存限額、可收回）、每票觸發 `rescore`、探索檔位、切換真實 Places API、交通時間細化、`dining_history`/`exposure_stats` 開始記錄、近期同席降權。
